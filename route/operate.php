...
switch ($action) {
    ...
    case 'move':
        if ('GET' == $method) {
            ...
            //更新tag fid
            //  echo "tagTids=$tagTids";
            if($tagTids!=''){
                // and tag_thread_big_update($tagTids,array('fid' => $newfid));  
                $db = $_SERVER['db'];
                $sql="UPDATE {$db->tablepre}website_tag_thread SET fid =  $newfid WHERE tid IN ($tagTids)";
    ...
    case 'search':
        ...
        //fidFind="+fid+"&fnameFind="+fname
        $keyword = param('keyword');
        $fidFInd = param('fidFind');
        $fnameFind = param('fnameFind');
        ...
        if ($keyword) {
            ...
            if ('like' == $search_type) {
                ...
                if (1 == $range) {
                    $threadlist = well_thread_find_by_keyword($keyword_decode,$fidFInd);
                }
