...
if ('POST' == $method) {
    ...
    foreach ($classList as $class) {
        ...
        if($tags==$class['name']){
            $fid=$class['fid'];
            // $flag_forum=$class['forum'];
            // $priceArr=$class['price'];
            // $uid = $class['uid'];
            // if($priceArr){
            //     if($priceArr==='Jewelry'){
            //         $needle = "earring";
            //         if (stripos($message, $needle) !== false) {
            //             $price=random_int(115, 229);
            //         } else {
            //             $price=random_int(59, 69);
            //         }
            //     }else{
            //          $price=random_int($priceArr[0],$priceArr[1]);
            //     }
            // }
            break;
        }
    ...
    $keyword=$tags;
    $brand=null;
    $key = array_search($tags, $nameList);
    echo "$tags --- $key";
    if($key){
      $brand=$newBrandList[$key];
    }
    if($brand==null){
        foreach ($brandList as $bd) {
        // $bArr = explode(",", $brand['findName']);
        foreach ($bd['findName'] as $b) {
            if (stripos($subject, $b) !== false) {
                $brand =  $bd['keyword'];
                break 2;
            }
        }
    ...
    echo '<br/>$flag_forum>>'.$flag_forum;
    echo '<br/>$flag_forum>>'.$image_url;
    // exit('发布测试');
