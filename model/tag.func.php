...
function well_tag_format(&$val)
{
    ...
    $val['url'] = url('brands-' . $val['tagid'], '', FALSE);
    // hook model__tag_format_end.php
}

//--------------------------cache--------------------------

function well_tag_read_by_tagid_cache($tagid,$fid=null)
{
    global $conf;
    // hook model__tag_read_by_tagid_cache_start.php
    $key = 'web_tag_' . $tagid;
    if($fid){
        $key = 'web_tag_' . $tagid.'_'.$fid;
    ...
    if ($new_tags) {
        ...
        foreach ($new_tags as $name) {
            ...
            if ($name && $i <= $n) {
                ...
                $tag_thread = array('tagid' => $read['tagid'], 'tid' => $tid,'fid' => $fid);
