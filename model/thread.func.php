...
function thread_create_handle($arr)
{
    ...
    $image_url = array_value($arr, 'image_url');
    $price = array_value($arr, 'price');
    ...
    $views=rand(3,8);
    $well_favorites=rand(0,1);


    // hook model__thread_create_before.php

    // 创建主题
    $thread = array('fid' => $fid, 'subject' => $subject,'image_url' => $image_url, 'price' =>$price, 'type' => $type, 'brief' => $brief, 'uid' => $uid, 'create_date' => $time, 'closed' => $closed, 'keyword' => $keyword, 'description' => $description, 'last_date' => $time, 'userip' => $longip, 'flags' => $flags, 'views' => $views, 'well_favorites' => $well_favorites);
...
function well_thread_inc_views($tid, $n = 1)
{
    ...
    $r = db_exec("UPDATE$sqladd `{$tablepre}website_thread` SET views=views+$n,well_favorites=well_favorites+$well_favorites WHERE tid='$tid'");
...
// 搜索标题
function well_thread_find_by_keyword($keyword,$fid, $d = NULL)
{
