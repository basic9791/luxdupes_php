...
/*--------- eval script start ----------*/
/*获取当前已经加载的 js*/
xn_bg.get_loaded_script = function () {
    var arr = [];
...
xn_bg.get_stylesheet_link = function (s) {
    var arr = [];
...
xn_bg.get_script_src = function (s) {
    var arr = [];
...
xn_bg.get_script_section = function (s) {
    var arr = s.match(/<script[^>]+ajax-eval="true"[^>]*>([\s\S]+?)<\/script>/ig);
    return arr ? arr : [];
};

xn_bg.strip_script_src = function (s) {
    s = s.replace(/<script[^>]*?src=\s*\"([^"]+)\"[^>]*><\/script>/ig, '');
    return s;
};

xn_bg.strip_script_section = function (s) {
    s = s.replace(/<script([^>]*)>([\s\S]+?)<\/script>/ig, '');
    return s;
};

xn_bg.strip_stylesheet_link = function (s) {
    s = s.replace(/<link[^>]*?href=\s*\"([^"]+)\"[^>]*>/ig, '');
    return s;
};

xn_bg.eval_script = function (arr, args) {
    if (!arr) return;
...
xn_bg.eval_stylesheet = function (arr) {
    if (!arr) return;
...
xn_bg.get_title_body_script_css = function (s) {
    var s = $.trim(s);
    ...
    var title = '';
    var body = '';
    var script_sections = xn_bg.get_script_section(s);
    var stylesheet_links = xn_bg.get_stylesheet_link(s);

    var arr1 = xn_bg.get_loaded_script();
    var arr2 = xn_bg.get_script_src(s);
    var script_srcs = xn_bg.array_diff(arr2, arr1);
    /*避免重复加载 js*/

    s = xn_bg.strip_script_src(s);
    s = xn_bg.strip_script_section(s);
    s = xn_bg.strip_stylesheet_link(s);
    ...
    $.xget(url, function (code, message) {
        ...
        if (r.script_srcs.length > 0) {
            $.require(r.script_srcs, function () {
                xn_bg.eval_script(r.script_sections, {jmodal: jmodal, callback: callback, arg: arg});
            });
        } else {
            xn_bg.eval_script(r.script_sections, {jmodal: jmodal, callback: callback, arg: arg});
        }
