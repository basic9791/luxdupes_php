...
xn_bg.form_radio = function(name, arr, checked) {
	var checked = checked || 0;
	if(xn_bg.empty(arr)) arr = [lang.no, lang.yes];
	var s = '';
...
xn_bg.form_options = function(arr, checked) {
	var checked = checked || 0;
	var s = '';
	$.each(arr, function(k, v) {
...
xn_bg.form_select = function(name, arr, checked, id) {
	var checked = checked || 0;
	var id = id || true;
	if(xn_bg.empty(arr)) return '';
	var idadd = id === true ? "id=\""+name+"\"" : (id ? "id=\""+id+"\"" : '');
	var s = '';
	s += "<select name=\""+name+"\" class=\"custom-select\" "+idadd+"> \r\n";
	s += xn_bg.form_options(arr, checked);
	s += "</select> \r\n";
	return s;
}