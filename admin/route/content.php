    ...
    case 'update':
        ...
        if ('GET' == $method) {
            ...
            if(9 == $thread['type']){
                 ...
                 $form_vote = 'checked="checked"';
                 $voteData= str_replace('&quot;','"', $thread_data['message']);
                 $jsonData=json_decode( $voteData,true);
  
                 $form_message =$jsonData['message'];
                 $thread_data['message']=$form_message;
            ...
            if (!empty($upload_thumbnail) || $thumbnail) {
                ...
                if ($thumbnail) {
                    ...
                    } elseif (preg_match_all('#<img[^>]+src="(.*?)"#i', $message, $match)) { //获取图片
                        $arr['icon'] = $time;
                        $i = 0;
